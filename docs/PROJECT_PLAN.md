...
확정 환경은 Windows, Python/Streamlit, SQLite다. API 키와 매물 제공처는 아직 없으므로 데모 실행과 실데이터 연동 구조를 먼저 구현한다.
...
데모는 서울·분당·부산의 가상 단지 10개, 거래 720건, 매물 100개와 과거 스냅샷 100개로 구성한다.
...
| 단계 | 예상 | 산출물·종료 조건 |
|---|---|---|
| 1. MVP 구성 | 이번 작업 | 계획 문서, 수집·DB·지도·데모·테스트·Windows 실행 파일 |
...
- API 키 없이 데모 첫 화면과 주요 필터가 오류 없이 실행되어야 한다.
- 실제 모드는 데모와 혼합되지 않으며 빈 상태에서도 수집 화면을 사용할 수 있어야 한다.

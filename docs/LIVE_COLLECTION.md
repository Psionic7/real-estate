...
원천동 전체를 포함하는 편의상 범위로, 광교신도시의 정확한 경계나 행정동 경계와 일치하지 않는다. 로컬 관리자 화면에서 법정동을 변경할 수 있다.
...
1. Windows에서 `run-admin.bat`을 실행하고 `http://127.0.0.1:8502`에 접속한다.
2. ‘수집 지역’ 메뉴에서 기존 지역 또는 ‘새 지역 직접 입력’을 선택한다.
3. 지역코드, 시도·시군구 전체 이름, 표시 이름, 법정동 범위를 입력한다. 법정동이 비어 있으면 구 전체다.
4. ‘매일 지정 시각’ 또는 ‘일정 간격’을 선택하고 수집 개월 수를 설정한 후 저장한다.
5. ‘실거래 수집’ 메뉴에서 계약월 범위를 입력하고 ‘선택 지역 지금 수집’을 누른다. ‘저장한 모든 지역 지금 수집’도 가능하다.
6. 작업 상태는 5초마다 갱신된다. 완료 후 조회 앱을 새로고침한다.

지역 삭제는 예약과 대기 작업만 제거하며 이미 저장한 실거래·매물은 보존한다. 코드가 다른 지역으로 변경되면 기존 설정을 삭제하고 새 지역으로 등록한다. 배포본의 `app.py`에는 관리자 메뉴나 수집 워커가 없다.
...
- 로컬 관리자 서버가 살아 있으면 별도 스레드가 대기열을 처리한다. 브라우저를 닫아도 관리자 서버 프로세스가 살아 있으면 계속 실행된다.
